...
const handleFileUploads = async (files, type) => {
  ...
  if (type === "a") {
    if (files.aeoCertificateFiles) {
      fileData.aeoCertificateFiles = await uploadAndMapMultipleFiles(
        files.aeoCertificateFiles
      );
    }
    if (files.siteListFiles) {
      fileData.siteListFiles = await uploadAndMapMultipleFiles(
        files.siteListFiles
      );
    }
    if (files.siteDetailsFiles) {
      fileData.siteDetailsFiles = await uploadAndMapMultipleFiles(
        files.siteDetailsFiles
      );
    }
    if (files.panFile) {
    ...
    if (files.enterpriseEvidenceFile) {
      fileData.enterpriseEvidenceFile = await uploadAndMapSingleFile(
        files.enterpriseEvidenceFile
      );
    }
    if (files.signatureFile) {
      fileData.signatureFile = await uploadAndMapSingleFile(
        files.signatureFile
      );
    }
  } else if (type === "b") {
    if (files.proceduralSecurityFile) {
      fileData.proceduralSecurityFile = await uploadAndMapSingleFile(
        files.proceduralSecurityFile
      );
    }
    if (files.documentSecurityFile) {
      fileData.documentSecurityFile = await uploadAndMapSingleFile(
        files.documentSecurityFile
      );
    }
    if (files.physicalSecurityFile) {
      fileData.physicalSecurityFile = await uploadAndMapSingleFile(
        files.physicalSecurityFile
      );
    }
    if (files.accessControlFile) {
      fileData.accessControlFile = await uploadAndMapSingleFile(
        files.accessControlFile
      );
    }
    if (files.personnelSecurityFile) {
      fileData.personnelSecurityFile = await uploadAndMapSingleFile(
        files.personnelSecurityFile
      );
    }
    if (files.trainingAndSkillUpgradationFile) {
      fileData.trainingAndSkillUpgradationFile = await uploadAndMapSingleFile(
        files.trainingAndSkillUpgradationFile
      );
    }
    if (files.govtSecurityComplianceFile) {
      fileData.govtSecurityComplianceFile = await uploadAndMapSingleFile(
        files.govtSecurityComplianceFile
      );
    }
  } else if (type === "c") {
    ...
    if (url) {
      try {
        ...
        if (publicIdMatch && publicIdMatch[1]) {
          ...
          console.warn(
            `Could not extract public_id from URL for deletion: ${url}`
          );
        }
      } catch (error) {
...
const createAnnexure = async (req, res) => {
  try {
    ...
    //     .json({
    //       error:
    //         "Annexure for this application and user already exists. Use PUT to update.",
    //     });
    // }
    ...
    const populatedAnnexure = await Model.findById(annexure._id)
      // 'applicationId',
      .populate("applicationType status") // Populate relevant application fields
  ...
  try {
    ...
    if (!existingAnnexure) {
      ...
      return res
        ...
        .json({
          error:
            "Annexure not found or you do not have permission to update it.",
        });
    }
    ...
    if (fileData) {
      // Only proceed if new files were actually uploaded
      if (type === "a") {
        if (
          fileData.aeoCertificateFiles &&
          fileData.aeoCertificateFiles.length > 0
        ) {
        ...
        if (fileData.trainingAndSkillUpgradationFile) {
          await deleteFromCloudinary(
            existingAnnexure.trainingAndSkillUpgradationFile
          );
        }
        if (fileData.govtSecurityComplianceFile) {
          await deleteFromCloudinary(
            existingAnnexure.govtSecurityComplianceFile
          );
        }
      }
    ...
    const updated = await Model.findOneAndUpdate(
      { userId: req.user.id }, // Find by annexure _id and userId
    ...
    const populatedAnnexure = await Model.findById(updated._id)
      .populate("applicationType status")
...
const getAnnexure = async (req, res) => {
  try {
    ...
    //   return res
    //     .status(400)
    //     .json({ error: "Annexure ID is required to retrieve." });
    // }
    ...
    if (!annexure) {
      return res.status(404).json({
        error: "Annexure not found or you do not have permission to access it.",
      });
    }
...
const deleteAnnexure = async (req, res) => {
  try {
    ...
    if (!annexureId) {
      return res
        .status(400)
        .json({ error: "Annexure ID is required for deletion." });
    }
    ...
    if (!annexure) {
      return res.status(404).json({
        error: "Annexure not found or you do not have permission to delete it.",
      });
    }
